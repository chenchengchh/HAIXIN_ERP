...
public class ScmPoEventRedisStreamJob {
    ...
    @Scheduled(fixedDelayString = "${wms.integration.scm-po-events.poll-delay-ms:1000}", initialDelayString = "${wms.integration.scm-po-events.initial-delay-ms:30000}")
    public void poll() {
        try {
            ...
            for (MapRecord<String, String, String> record : records) {
                ...
                try {
                    ...
                    log.warn("WMS 处理 SCM PO event 失败 recordId={} error={}", record.getId(), ex.getMessage());
                }
            }
        } catch (Exception e) {
            log.warn("WMS 轮询 SCM PO stream 失败 error={}", e.getMessage());
        }
    }
