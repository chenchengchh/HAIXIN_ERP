...
public class WarehouseLocationMirrorService {
    ...
    public void applyWarehouseEvent(WarehouseEventRequest req) {
        ...
        if (eventType.toUpperCase().contains("DELETED")) {
            ...
            jdbcTemplate.update(
                    "UPDATE erp_warehouse SET is_deleted=1, status=0, updated_time=NOW() WHERE warehouse_code=:warehouseCode",
                    params
            );
            return;
        }
    ...
    public void applyLocationEvent(LocationEventRequest req) {
        ...
        if (eventType.toUpperCase().contains("DELETED")) {
            ...
            jdbcTemplate.update("DELETE FROM erp_location WHERE location_code=:locationCode", params);
            return;
        }
    ...
    private void upsertWarehouse(WarehouseEventRequest.WarehousePayload w, String eventType) {
        ...
        params.put("warehouseType", null);
        params.put("location", w.getAddress());
        params.put("manager", w.getManager());
        params.put("status", w.getStatus() == null ? 1 : w.getStatus());
        params.put("remark", eventType);

        if (existingId == null) {
            params.put("isDeleted", 0);
            jdbcTemplate.update(
                    "INSERT INTO erp_warehouse (warehouse_code, warehouse_name, warehouse_type, location, manager, status, remark, created_time, updated_time, is_deleted) "
                            + "VALUES (:warehouseCode, :warehouseName, :warehouseType, :location, :manager, :status, :remark, NOW(), NOW(), :isDeleted)",
                    params
            );
        } else {
            params.put("id", existingId);
            jdbcTemplate.update(
                    "UPDATE erp_warehouse SET warehouse_name=:warehouseName, location=:location, manager=:manager, status=:status, remark=:remark, is_deleted=0, updated_time=NOW() "
                            + "WHERE id=:id",
                    params
            );
        }
    ...
    private void upsertLocation(LocationEventRequest.LocationPayload l, String eventType) {
        ...
        if (existingId == null) {
            jdbcTemplate.update(
                    "INSERT INTO erp_location (location_code, location_name, warehouse_code, zone_code, location_type_code, status, remark, created_time, updated_time) "
            ...
            params.put("id", existingId);
            jdbcTemplate.update(
                    "UPDATE erp_location SET location_name=:locationName, warehouse_code=:warehouseCode, zone_code=:zoneCode, location_type_code=:locationTypeCode, status=:status, remark=:remark, updated_time=NOW() "
                            + "WHERE id=:id",
                    params
            );
        }
    ...
    private Long findWarehouseId(String warehouseCode) {
        List<Long> ids = jdbcTemplate.query(
                "SELECT id FROM erp_warehouse WHERE warehouse_code=:warehouseCode LIMIT 1",
    ...
    private Long findLocationId(String locationCode) {
        List<Long> ids = jdbcTemplate.query(
                "SELECT id FROM erp_location WHERE location_code=:locationCode LIMIT 1",
