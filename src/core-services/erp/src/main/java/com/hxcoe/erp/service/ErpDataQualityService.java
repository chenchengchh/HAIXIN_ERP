...
@Service
public class ErpDataQualityService {
    ...
    public Map<String, Object> refresh() {
        ...
        int missingUnit = refreshMaterialMissingUnit();
        int missingSpec = refreshMaterialMissingSpec();
        int metrics = refreshMetrics();
        res.put("materialMissingUnitUpserts", missingUnit);
        res.put("materialMissingSpecUpserts", missingSpec);
    ...
    public PageResult<Map<String, Object>> listTasks(Integer page, Integer size, String status) {
        ...
        Long total = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM erp_data_clean_task " + where,
                params,
                Long.class
        );
        if (total == null) {
    ...
    public boolean resolveTask(Long id, String resolution, String status) {
        ...
        int updated = jdbcTemplate.update(
                "UPDATE erp_data_clean_task SET status=:status, resolution=:resolution, updated_time=NOW() WHERE id=:id",
                params
        );
        return updated > 0;
    }
    ...
    private int refreshMaterialMissingUnit() {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT id, material_code, material_name, unit, specification "
                        + "FROM erp_material "
        ...
        for (Map<String, Object> row : rows) {
            String materialCode = row.get("material_code") == null ? "" : String.valueOf(row.get("material_code"));
            if (materialCode.isBlank()) {
                continue;
            }
            upserts += upsertTask("MATERIAL", "MISSING_UNIT", materialCode, row);
        }
        return upserts;
    }

    private int refreshMaterialMissingSpec() {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT id, material_code, material_name, unit, specification "
                        + "FROM erp_material "
        ...
        for (Map<String, Object> row : rows) {
            ...
            upserts += upsertTask("MATERIAL", "MISSING_SPECIFICATION", materialCode, row);
        }
        return upserts;
    }
    ...
    private int refreshMetrics() {
        ...
        long materialTotal = nvlLong(jdbcTemplate.queryForObject("SELECT COUNT(*) FROM erp_material", new MapSqlParameterSource(), Long.class));
        long missingUnit = nvlLong(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM erp_material WHERE unit IS NULL OR TRIM(unit) = ''",
                new MapSqlParameterSource(),
                Long.class
        ));
        long missingSpec = nvlLong(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM erp_material WHERE specification IS NULL OR TRIM(specification) = ''",
                new MapSqlParameterSource(),
                Long.class
        ));

        int upserts = 0;
        upserts += upsertMetric(d, "material.total", materialTotal, null);
        upserts += upsertMetric(d, "material.missing_unit", missingUnit, null);
        upserts += upsertMetric(d, "material.missing_specification", missingSpec, null);
        return upserts;
    }
    ...
    private int upsertTask(String entityType, String conflictType, String businessKey, Map<String, Object> detail) {
        ...
        return jdbcTemplate.update(
                "INSERT INTO erp_data_clean_task (entity_type, conflict_type, business_key, detail_json, status, created_time, updated_time) "
    ...
    private int upsertMetric(LocalDate date, String key, long value, Map<String, Object> detail) {
        ...
        params.addValue("detailJson", json);
        return jdbcTemplate.update(
                "INSERT INTO erp_data_quality_metric (metric_date, metric_key, metric_value, detail_json, created_time) "
    ...
    private long countTasks(String status) {
        if (status == null || status.isBlank()) {
            Long v = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM erp_data_clean_task", new MapSqlParameterSource(), Long.class);
            return nvlLong(v);
        }
        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("status", status);
        Long v = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM erp_data_clean_task WHERE status=:status", params, Long.class);
        return nvlLong(v);
    }
}
