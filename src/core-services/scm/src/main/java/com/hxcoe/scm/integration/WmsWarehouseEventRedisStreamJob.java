...
public class WmsWarehouseEventRedisStreamJob {
    ...
    public void poll() {
        ...
        String stream = integrationTransportProperties.getRedis().getStreamWmsWarehouseEvents();
        String group = integrationTransportProperties.getRedis().getGroupWmsWarehouseEvents();
        String consumer = integrationTransportProperties.getRedis().getConsumerWmsWarehouseEvents();
        ...
        try {
            ...
            log.warn("SCM Redis Stream 读取失败 stream={} error={} nextRetryInMs={}", stream, ex.getMessage(), delayMs);
            return;
        }
        ...
        for (MapRecord<String, Object, Object> r : records) {
            try {
                ...
                WarehouseEventRequest req = objectMapper.readValue(body, WarehouseEventRequest.class);
                warehouseLocationMirrorService.applyWarehouseEvent(req);
                ack(stream, group, r);
            } catch (Exception ex) {
                log.warn("SCM Redis Stream 消费失败 stream={} recordId={} error={}", stream, r.getId(), ex.getMessage());
            }
        }
    }
